...
import { Archive, ArrowLeft, Camera, Check, Clipboard, Download, History, Keyboard, Layers3, MapPin, MoveRight, ScanLine, Search, Smartphone, Trash2, X } from "lucide-react";
...
export function SampleDetailView({ id }: { id: string }) {
  const { lookups, can } = useAppData(); const router = useRouter(); const toast = useToast(); const { data, loading, error, reload } = useRemote<SampleDetailData>(`/api/samples/${encodeURIComponent(id)}`); const scanMoveOpenedRef = useRef(false); const [moveOpen, setMoveOpen] = useState(false); const [deleteOpen, setDeleteOpen] = useState(false); const [deleteReason, setDeleteReason] = useState(""); const [deleting, setDeleting] = useState(false);   const [form, setForm] = useState({ status: "active", departmentId: "", locationId: "", remark: "", note: "", spec: "" }); const [saving, setSaving] = useState(false); const [copied, setCopied] = useState(false);
  ...
  async function deleteSample(event: FormEvent) {
    event.preventDefault();
    if (!deleteReason.trim()) { toast("请填写删除原因", "error"); return; }
    setDeleting(true);
    try {
      await apiFetch(`/api/samples/${s.id}`, { method: "DELETE", body: JSON.stringify({ reason: deleteReason.trim() }) });
      toast("样品已删除");
      setDeleteOpen(false);
  ...
  return <>
    ...
    {deleteOpen && <Modal title={`删除样品 ${s.code}`} onClose={() => { if (!deleting) setDeleteOpen(false); }}><form className="modal-form" onSubmit={deleteSample}><p style={{margin:"0 0 12px",fontSize:"14px",color:"var(--muted)"}}>此操作将从系统中移除该样品及其流转记录。请填写删除原因以便后续追溯。</p><Field label="删除原因" required><textarea rows={3} required maxLength={500} value={deleteReason} onChange={(event) => setDeleteReason(event.target.value)} placeholder="例如：录入错误、样品已报废处理等" /></Field><div className="modal-actions"><button type="button" className="button button-ghost" disabled={deleting} onClick={() => setDeleteOpen(false)}>取消</button><button className="button button-primary" style={{background:"var(--red)",borderColor:"var(--red)"}} disabled={deleting || !deleteReason.trim()}>{deleting ? "删除中…" : "确认删除"}</button></div></form></Modal>}
  </>;
}
