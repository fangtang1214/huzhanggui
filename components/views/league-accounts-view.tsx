...
type LeagueAccount = { id: string; name: string; appid: string; active: boolean; isPrimary: boolean; createdAt: string; createdByName?: string | null; directoryItemCount?: number | null; directorySyncedAt?: string | null; directorySyncStatus: "idle" | "pending" | "running" | "failed"; directorySyncError?: string | null; directorySyncStartedAt?: string | null; directorySyncProgressCount?: number | null; directorySyncHeartbeatAt?: string | null; };
    ...
    <PageHeader eyebrow="系统管理" title="联盟带货机构" description="本地仅保存商品 ID 与机构推广标识；商品资料、推广链接和服务费率在使用时通过接口获取最新值。" actions={<button className="button button-primary" onClick={openNew}><Plus size={17} />添加机构账号</button>} />
     <section className="panel table-panel">
       {loading ? <LoadingState /> : error ? <ErrorState message={error} retry={reload} /> : !data?.length ? <EmptyState title="尚未配置机构账号" description="在微信开发者平台注册联盟带货机构后，把 AppID 和密钥添加到这里。" /> : <div className="data-table-wrap"><table className="data-table"><thead><tr><th>账号名称</th><th>机构 AppID</th><th>合作商品目录</th><th>添加时间</th><th>状态</th><th /></tr></thead><tbody>{data.map((account) => <tr key={account.id}>
        ...
        <td><b>{account.directorySyncStatus === "running" ? `已保存 ${account.directoryItemCount ?? 0} 件` : `${account.directoryItemCount ?? 0} 件`}</b><small style={{ display: "block", marginTop: 3, color: account.directorySyncStatus === "failed" ? "var(--red)" : "var(--muted)" }}>{directoryStatusLabels[account.directorySyncStatus]}{account.directorySyncStatus === "running" ? ` · 已扫描 ${account.directorySyncProgressCount ?? 0} 件${account.directorySyncStartedAt ? ` · 开始于 ${formatDate(account.directorySyncStartedAt, true)}` : ""}` : account.directorySyncedAt ? ` · ${formatDate(account.directorySyncedAt, true)}` : " · 尚未完成首次同步"}</small>{account.directorySyncError && <small style={{ display: "block", maxWidth: 320, color: "var(--red)" }}>{account.directorySyncError}</small>}</td>
    ...
    {editing && <Modal title={editing === "new" ? "添加机构账号" : `编辑机构账号 ${editing.name}`} onClose={() => { if (!saving) setEditing(null); }}>
      <form className="modal-form" onSubmit={submit}>
        ...
        <Field label={editing === "new" ? "AppSecret" : "AppSecret（留空则不修改）"} required={editing === "new"} hint="密钥仅保存在服务器数据库中。"><input type="password" required={editing === "new"} maxLength={200} autoCapitalize="none" spellCheck={false} value={draft.appSecret} onChange={(event) => setDraft({ ...draft, appSecret: event.target.value.trim() })} placeholder={editing === "new" ? "" : "不修改请留空"} /></Field>
         ...
         <Field label="主账号" hint="最多只能设置一个；同为机构分配链接或同为商家指定链接时，优先使用主账号。"><label className="check-card" style={{ display: "inline-flex", gap: 8, padding: "8px 12px" }}><input type="checkbox" checked={draft.isPrimary} disabled={!draft.active} onChange={(event) => setDraft({ ...draft, isPrimary: event.target.checked })} /><span>{draft.isPrimary ? "主账号" : "设为主账号"}</span></label></Field>
