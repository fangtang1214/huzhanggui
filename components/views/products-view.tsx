...
import { Archive, ArrowDownUp, ArrowLeft, Boxes, CalendarDays, CheckCircle2, ChevronDown, ChevronUp, CircleAlert, Clipboard, Download, ExternalLink, FilePenLine, Filter, GripVertical, ImagePlus, LoaderCircle, MapPin, PackagePlus, Plus, RefreshCw, Search, Settings2, Sparkles, Store, TriangleAlert, UserRound } from "lucide-react";
...
export function ProductsView() {
  const { lookups, can, user } = useAppData(); const router = useRouter(); const toast = useToast(); const [search, setSearch] = useState(""); const [departmentId, setDepartmentId] = useState(""); const [categoryId, setCategoryId] = useState(""); const [prices, setPrices] = useState<string[]>([]); const [priceOrder, setPriceOrder] = useState<"" | "asc" | "desc">(""); const [page, setPage] = useState(1);
  ...
  const queryParams = new URLSearchParams({ search, departmentId, categoryId, page: String(page) });
  ...
  function toggleCopyField(key: ProductCopyFieldKey) {
    setCopyDraft((config) => ({ ...config, enabled: config.enabled.includes(key) ? config.enabled.filter((field) => field !== key) : [...config.enabled, key] }));
  }
  function reorderCopyField(source: ProductCopyFieldKey, target: ProductCopyFieldKey) {
    if (source === target) return;
    setCopyDraft((config) => {
      const order = config.order.filter((field) => field !== source); const targetIndex = order.indexOf(target);
  ...
  function moveCopyField(key: ProductCopyFieldKey, direction: -1 | 1) {
    setCopyDraft((config) => {
      const order = [...config.order]; const index = order.indexOf(key); const next = index + direction;
  ...
  return <>
    <PageHeader eyebrow="商品档案" title="所有商品款式" description="一个基础货号对应一个款式，每件实物在基础货号后追加独立序号。" actions={<><button className="button button-secondary" type="button" onClick={openCopySettings}><Settings2 size={17} />复制设置</button>{can("products:export") && <Link className="button button-secondary" href="/api/export?type=products"><Download size={17} />导出 Excel</Link>}{can("products:create") && <Link className="button button-primary" href="/products/new"><PackagePlus size={18} />登记新商品</Link>}</>} />
    ...
    {copySettingsOpen && <Modal title="一键复制设置" onClose={() => { if (!savingCopyConfig) setCopySettingsOpen(false); }} wide><div className="copy-settings"><div className="copy-settings-intro"><Settings2 size={20} /><div><b>选择并排列复制内容</b><p>拖动字段调整顺序；复制商品后，粘贴到腾讯文档、飞书或 WPS 表格时会按此顺序分配到相邻单元格。</p></div></div><div className="copy-field-list">{copyDraft.order.map((key, index) => { const field = PRODUCT_COPY_FIELDS.find((item) => item.key === key); if (!field) return null; const checked = copyDraft.enabled.includes(key); return <div className={`copy-field-item ${checked ? "selected" : ""} ${draggingField === key ? "dragging" : ""}`} draggable onDragStart={() => setDraggingField(key)} onDragEnd={() => setDraggingField(null)} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); if (draggingField) reorderCopyField(draggingField, key); setDraggingField(null); }} key={key}><GripVertical className="copy-drag-handle" size={18} /><label><input type="checkbox" checked={checked} onChange={() => toggleCopyField(key)} /><span>{field.label}</span></label><div><button type="button" className="icon-button" disabled={index === 0} onClick={() => moveCopyField(key, -1)} aria-label={`上移${field.label}`}><ChevronUp size={16} /></button><button type="button" className="icon-button" disabled={index === copyDraft.order.length - 1} onClick={() => moveCopyField(key, 1)} aria-label={`下移${field.label}`}><ChevronDown size={16} /></button></div></div>; })}</div><div className="modal-actions"><span className="copy-selected-count">已选择 {copyDraft.enabled.length} 项</span><button type="button" className="button button-ghost" disabled={savingCopyConfig} onClick={() => setCopySettingsOpen(false)}>取消</button><button type="button" className="button button-primary" disabled={savingCopyConfig || !copyDraft.enabled.length} onClick={saveCopyConfig}>{savingCopyConfig ? "保存中…" : "保存设置"}</button></div></div></Modal>}
  </>;
}
  ...
  return <form onSubmit={submit}>
    ...
    {(id || imageUrls.length > 0) && <div className="form-layout"><div className="form-main">
      ...
      <section className="panel form-section"><header><span className="section-number">03</span><div><h2>店铺与合作信息</h2><p>这些字段可后续修改，商品链接变更会记录操作人和时间。</p></div></header><div className="form-grid">
        ...
        <Field label="商品链接" hint="支持完整网址，也支持 weixinstorehs/28512353738164 这类视频号链接。" className="field-full"><div className="input-prefix"><ExternalLink size={17} /><input type="text" inputMode="url" autoCapitalize="none" spellCheck={false} value={form.productUrl} onChange={(event) => set("productUrl", event.target.value)} placeholder="https://... 或 weixinstorehs/..." /></div></Field>
      </div></section>
