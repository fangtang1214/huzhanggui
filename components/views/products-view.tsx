...
type ProductRow = { id: string; sku: string; name: string; archived?: boolean; storeName?: string; price?: string; productUrl?: string; apiProductId?: string | null; apiOutProductId?: string | null; commission?: string; storeRating?: string; supplyChain?: string; cooperationMechanism?: string; notes?: string; imageUrls: string[]; createdAt: string; updatedAt?: string; categoryName?: string; businessContactName?: string; sampleCount: number; activeCount: number; selectedDepartments?: string; tags?: string; pendingIssueId?: string; latestResolvedIssueId?: string; latestResolvedAt?: string };
...
function ProductApiIdSummary({ rows }: { rows: ProductRow[] }) {
  const items = rows.filter((row) => row.apiProductId || row.apiOutProductId);
  if (!items.length) return null;
  return <section className="panel" style={{ padding: "12px 16px", marginBottom: 12 }}><div className="product-id-summary"><b>当前商品 ID</b>{items.map((row) => <Link href={`/products/${row.id}`} key={row.id}><span>{row.name}</span><code>{row.apiProductId || "—"}{row.apiOutProductId ? ` · out: ${row.apiOutProductId}` : ""}</code></Link>)}</div></section>;
}
...
<select value={storageDepartmentId} onChange={(event) => { setStorageDepartmentId(event.target.value); setPage(1); }}><option value="">全部存放位置</option>{lookups?.departments.map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</select>
     ...
     <ProductApiIdSummary rows={data?.rows || []} />
...
export function ProductFormView({ id }: { id?: string }) {
  ...
  function chooseCandidate(candidate: MatchCandidate) {
    ...
    setRecognition((value) => ({ ...value, phase: "ready", decision: "matched", matchedProductId: candidate.id, message: form.windowProductId ? `已确认与 ${candidate.sku} 为同款；将沿用货号并更新橱窗店铺与合作信息` : `已确认与 ${candidate.sku} 为同款，本次将沿用该货号` }));
