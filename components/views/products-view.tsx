...
function ExpandedProductSamples({ productId }: { productId: string }) {
  ...
  return <div style={{ padding: "12px 0" }}><table className="data-table" style={{ fontSize: 13 }}><thead><tr><th>独立编号</th><th>到样日期</th><th>状态</th><th>当前位置</th><th>最后更新</th><th /></tr></thead><tbody>{samples.map((sample) => <tr key={sample.id}><td><Link href={`/samples/${sample.code}`} className="code-link">{sample.code}</Link></td><td>{formatDate(sample.arrivedAt)}</td><td><StatusBadge status={sample.status} /></td><td><span className="table-place"><MapPin size={14} />{activeLocationLabel({ status: sample.status, department_name: sample.departmentName, location_name: sample.locationName })}</span></td><td>{formatDate(sample.updatedAt, true)}</td><td><Link className="row-link" href={`/samples/${sample.code}`}>查看流转</Link></td></tr>)}</tbody></table></div>;
}

export function ProductsView() {
  const { lookups, can, user } = useAppData(); const router = useRouter(); const toast = useToast();   const [search, setSearch] = useState(""); const [departmentId, setDepartmentId] = useState(""); const [categoryId, setCategoryId] = useState(""); const [prices, setPrices] = useState<string[]>([]); const [priceOrder, setPriceOrder] = useState<"" | "asc" | "desc">(""); const [page, setPage] = useState(1); const [expandedIds, setExpandedIds] = useState<string[]>([]);
