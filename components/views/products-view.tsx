  ...
  if (archiveView) return <>
    ...
    <section className="toolbar">
      ...
      <div className="toolbar-filters">
        ...
        <select value={locationId} onChange={(event) => { setLocationId(event.target.value); setPage(1); }}><option value="">全部存放位置</option>{lookups?.locations.map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</select>
      </div>
    </section>
