...
import { activeLocationLabel, SAMPLE_STATUSES } from "@/lib/constants";
...
type MatchedSamplesData = { product: { id: string; sku: string; name: string; archived: boolean }; samples: SampleRow[] };
...
function MatchedProductSamples({ productId, sku }: { productId: string; sku: string }) {
  const { data, loading, error, reload } = useRemote<MatchedSamplesData>(`/api/products/${productId}/intake-samples`);
  const archived = Boolean(data?.product.archived);
  const samples = data?.samples || [];
  return <section className="panel table-panel matched-samples-panel">
    <header className="panel-header padded"><div><p className="eyebrow">已确认关联同款</p><h2>该款实物样品{data ? `（${samples.length}）` : ""}</h2><small>{data ? `${data.product.sku} · ${data.product.name}${archived ? " · 商品已归档" : ""}` : `货号 ${sku}`}</small></div><Link className="row-link" href={`/products/${productId}`} target="_blank" rel="noreferrer">打开商品档案</Link></header>
    {loading ? <LoadingState label="正在读取该款实物样品…" /> : error ? <ErrorState message={error} retry={reload} /> : !samples.length ? <EmptyState title="该款目前没有实物样品" description="可以继续填写本次到样信息，提交后会创建新的实物编码。" /> : <>
      <div className="data-table-wrap"><table className="data-table"><thead><tr><th>实物编码</th><th>规格</th><th>状态</th><th>{archived ? "最后位置" : "当前位置"}</th><th>最后更新</th><th /></tr></thead><tbody>{samples.map((sample) => <tr key={sample.id}><td><Link href={`/samples/${sample.code}`} target="_blank" rel="noreferrer" className="code-link">{sample.code}</Link></td><td>{sample.spec || "—"}</td><td>{sample.archived ? <span className="soft-badge archived-badge">已归档</span> : <StatusBadge status={sample.status} />}</td><td><span className="table-place"><MapPin size={14} />{activeLocationLabel({ status: sample.status, department_name: sample.departmentName, location_name: sample.locationName })}</span></td><td>{formatDate(sample.updatedAt, true)}</td><td><Link className="row-link" href={`/samples/${sample.code}`} target="_blank" rel="noreferrer">查看流转</Link></td></tr>)}</tbody></table></div>
      <div className="mobile-record-list">{samples.map((sample) => <div className="mobile-record compact" key={sample.id}><div><Link href={`/samples/${sample.code}`} target="_blank" rel="noreferrer" className="code-link">{sample.code}</Link><p><MapPin size={14} />{activeLocationLabel({ status: sample.status, department_name: sample.departmentName, location_name: sample.locationName })}</p><small>{sample.spec || "未填写规格"} · {formatDate(sample.updatedAt, true)}</small></div>{sample.archived ? <span className="soft-badge archived-badge">已归档</span> : <StatusBadge status={sample.status} />}</div>)}</div>
    </>}
  </section>;
}

export function ProductsView() {
  const { lookups, can, user } = useAppData(); const router = useRouter(); const toast = useToast();   const [search, setSearch] = useState(""); const [departmentId, setDepartmentId] = useState(""); const [categoryId, setCategoryId] = useState(""); const [storageDepartmentId, setStorageDepartmentId] = useState(""); const [productStatus, setProductStatus] = useState(""); const [prices, setPrices] = useState<string[]>([]); const [priceOrder, setPriceOrder] = useState<"" | "asc" | "desc">(""); const [page, setPage] = useState(1); const [expandedIds, setExpandedIds] = useState<string[]>([]);
  ...
  if (productStatus) queryParams.set("status", productStatus);
  ...
  if (archiveView) return <>
    ...
    <section className="toolbar">
      <div className="search-box"><Search size={18} /><input value={search} onChange={(event) => { setSearch(event.target.value); setPage(1); }} placeholder="搜索已归档商品的货号、商品 ID、名称、店铺或链接" /></div>
      <div className="toolbar-filters">
        ...
        <select value={productStatus} onChange={(event) => { setProductStatus(event.target.value); setPage(1); }}><option value="">全部商品状态</option>{SAMPLE_STATUSES.map((item) => <option value={item.value} key={item.value}>{item.label}</option>)}</select>
      </div>
    </section>
     <section className="panel table-panel archived-products-panel">
      {loading ? <LoadingState /> : error ? <ErrorState message={error} retry={reload} /> : !data?.rows.length ? <EmptyState title="没有已归档商品" description={search || departmentId || categoryId || storageDepartmentId || productStatus ? "没有符合当前筛选条件的归档记录。" : "归档的商品会集中显示在这里。"} /> : <>
...
export function ProductFormView({ id }: { id?: string }) {
  ...
  const [skuSuggestionVersion, setSkuSuggestionVersion] = useState(0);
  ...
  useEffect(() => {
    ...
    void apiFetch<SkuLookupResult>("/api/products/sku").then((result) => {
      ...
      setForm((current) => {
        ...
        const keepCurrent = /^\d{4}$/.test(currentSuffix) && currentSuffix !== "0000";
        return { ...current, sku: keepCurrent ? current.sku : result.suggestedSku || "" };
      });
  ...
  }, [draftReady, id, skuSuggestionVersion]);
  useEffect(() => {
  ...
  function changeRegistrationMode() {
    ...
    setSkuSuggestionLoaded(false); setSkuSuggestionVersion((value) => value + 1); setSkuAvailability({ status: "idle", message: "" });
  ...
  return <form onSubmit={submit}>
    ...
    {!id && recognition.decision === "matched" && recognition.matchedProductId && <MatchedProductSamples key={recognition.matchedProductId} productId={recognition.matchedProductId} sku={form.sku} />}
