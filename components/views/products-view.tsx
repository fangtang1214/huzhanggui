...
import { Archive, ArrowDownUp, ArrowLeft, Boxes, CalendarDays, CheckCircle2, ChevronDown, ChevronUp, CircleAlert, Clipboard, Download, ExternalLink, FilePenLine, FileSpreadsheet, Filter, GripVertical, History, ImagePlus, Link2, LoaderCircle, MapPin, PackagePlus, Plus, RefreshCw, Search, Settings2, Sparkles, Store, Trash2, TriangleAlert, Upload, UserRound, X } from "lucide-react";
...
export function ProductsView() {
  ...
  if (archiveView) return <>
    ...
    <section className="panel table-panel archived-products-panel">
      {loading ? <LoadingState /> : error ? <ErrorState message={error} retry={reload} /> : !data?.rows.length ? <EmptyState title="没有已归档商品" description={search || departmentId || categoryId ? "没有符合当前筛选条件的归档记录。" : "归档的商品会集中显示在这里。"} /> : <>
        <div className="data-table-wrap"><table className="data-table product-table"><thead><tr><th>商品</th><th>选品直播间</th><th>商务信息</th><th>价格</th><th>佣金</th><th>归档样品</th><th>归档时间</th><th /></tr></thead><tbody>{data.rows.map((product) => <tr className="archived-product-row" key={product.id}>
          <td><Link href={"/products/" + product.id} className="table-product"><ProductImage urls={product.imageUrls} alt={product.name} size="small" /><div><b>{product.name}</b><span>{product.sku} {product.categoryName && "· " + product.categoryName}</span>{product.tags && <small>{product.tags}</small>}</div></Link></td>
