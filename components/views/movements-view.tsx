...
import { EmptyState, ErrorState, LoadingState, PageHeader, Pagination, StatusBadge } from "../ui";
import { useState } from "react";

type Movement = { id: string; batchId?: string; fromStatus?: string; toStatus: string; remark?: string; createdAt: string; sampleId: string; code: string; sku: string; productName: string; fromDepartmentName?: string; fromLocationName?: string; toDepartmentName?: string; toLocationName?: string; operatorName?: string };
...
export function MovementsView() {
  ...
  return <>
    ...
    <section className="panel table-panel">{loading ? <LoadingState /> : error ? <ErrorState message={error} retry={reload} /> : !data?.rows.length ? <EmptyState title="暂无流转记录" /> : <><div className="movement-feed">{data.rows.map((item) => <Link href={`/samples/${item.code}`} className="movement-row" key={item.id}><div className="movement-time"><b>{formatDate(item.createdAt, true).split(" ").pop()}</b><span>{formatDate(item.createdAt)}</span></div><div className="movement-product"><span className="code-text">{item.code}</span><b>{item.productName}</b><small>货号 {item.sku}</small></div><div className="movement-path"><div><small>从</small><span>{place(item.fromStatus, item.fromDepartmentName, item.fromLocationName)}</span></div><MoveRight size={20} /><div><small>到</small><b>{place(item.toStatus, item.toDepartmentName, item.toLocationName)}</b></div></div><div className="movement-operator"><StatusBadge status={item.toStatus} /><span>{item.operatorName || "系统"}</span>{item.remark && <small>{item.remark}</small>}</div></Link>)}</div><Pagination page={data.page} pageSize={data.pageSize} total={data.total} onChange={setPage} /></>}</section>
  </>;
}
