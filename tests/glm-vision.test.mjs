...
import { _test, reviewCandidates } from "../lib/glm-vision.ts";
...
test("GLM 复核会使用全部新图片且每个历史候选最多三张图", async () => {
  ...
  try {
    const reviews = await reviewCandidates("test-key", [firstAcceptanceImage, secondAcceptanceImage], [{ id: "candidate-1", sku: "HZG-1", name: "腰包", imageUrls: ["https://old.example/1.jpg", "https://old.example/2.jpg", "https://old.example/3.jpg", "https://old.example/4.jpg"] }]);
