...
export const GLM_MODEL = "glm-4.6v-flash";
...
export async function getGlmSettings() {
  ...
  return { configured: Boolean(value.configured && value.apiKeyEncrypted), indexingStatus: value.indexingStatus || "idle", model: GLM_MODEL };
}
...
async function callGlm(apiKey: string, content: Array<Record<string, unknown>>, signal?: AbortSignal) {
  const response = await fetch(GLM_ENDPOINT, {
    method: "POST",
    headers: { authorization: `Bearer ${apiKey}`, "content-type": "application/json" },
    body: JSON.stringify({ model: GLM_MODEL, temperature: 0.05, messages: [{ role: "user", content }] }),
    signal,
  });
...
export async function testGlmConnection(apiKey: string, signal?: AbortSignal) {
  const text = await callGlm(apiKey, [{ type: "text", text: "仅回复 OK，用于检查模型连接。" }], signal);
  return Boolean(text.trim());
}

export async function analyzeSubject(apiKey: string, imageUrl: string, signal?: AbortSignal): Promise<SubjectBox> {
  const text = await callGlm(apiKey, [
...
export async function reviewCandidates(apiKey: string, newImageUrls: string[], candidates: Array<{ id: string; name: string; sku: string; imageUrls: string[] }>, signal?: AbortSignal): Promise<GlmReview[]> {
  ...
  const text = await callGlm(apiKey, content, signal);
