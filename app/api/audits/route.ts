...
import { auditSearchConditions } from "@/lib/search";

export async function GET(request: Request) {
  try {
    await requireUser("audits:view"); const sql = getDb(); const url = new URL(request.url);
    const search = (url.searchParams.get("search") || "").trim();
    const searchFrag = auditSearchConditions(sql, search);
