...
export async function GET() {
  try {
    await requireUser();
    const sql = getDb();
    const [departments, locations, categories, tags, users] = await Promise.all([
    ...
    return ok({ departments, locations, categories, tags, users, permissionGroups: PERMISSION_GROUPS });
  } catch (error) {
