...
export async function PATCH(request: Request, context: RouteContext) {
  try {
    ...
    const row = await sql.begin(async (tx) => {
      const [before] = await tx`SELECT id, name, appid, active, is_primary FROM league_accounts WHERE id = ${id} FOR UPDATE`;
      if (!before) return null;
      if (input.isPrimary && !input.active) {
        throw new AuthError("已停用的机构账号不能设为主账号", 400);
      }
      if (input.isPrimary) await tx`UPDATE league_accounts SET is_primary = false, updated_at = now() WHERE id <> ${id} AND is_primary = true`;
      const [updated] = await tx`
        UPDATE league_accounts
        SET name = ${input.name}, appid = ${input.appid}, active = ${input.active}, is_primary = ${input.isPrimary},
...
export async function DELETE(request: Request, context: RouteContext) {
  try {
    ...
    const { id } = await context.params;
    const sql = getDb();
    const [before] = await sql`SELECT name, appid, is_primary FROM league_accounts WHERE id = ${id}`;
    if (!before) return Response.json({ ok: false, message: "机构账号不存在" }, { status: 404 });
