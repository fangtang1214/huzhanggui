...
const schema = z.object({
  ...
  permissions: accountPermissionsSchema.default([]),
  ...
  try {
    ...
    const rows = await sql`
      SELECT u.id, u.username, u.name, u.department_id, d.name AS department_name,
             u.permissions, u.is_super_admin, u.active, u.must_change_password,
...
export async function POST(request: Request) {
  try {
    ...
    assertCanChangeAccountPermissions(user, [], input.permissions, false);
    const passwordHash = await bcrypt.hash(input.password, 12);
    const [row] = await sql`
      INSERT INTO users(username, name, password_hash, department_id, permissions, must_change_password)
      VALUES (${input.username.toLowerCase()}, ${input.name}, ${passwordHash}, ${input.departmentId}, ${sql.json(input.permissions)}, ${input.mustChangePassword})
      RETURNING id, username, name, department_id, permissions
    `;
